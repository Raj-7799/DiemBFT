...
class Safety():
    ...
    def _increase_highest_vote_round(self, roundNo):
        # self.OutputLogger("[replicaID {}] Block round {} is less than qc_round {} or high vote round {}".format(self.sender, block_round, qc_round, self.highest_vote_round))
    ...
    def _commit_state_id_candidate(self, block_round, qc):
        if self._consecutive(block_round, qc.vote_info.roundNo):
            ...
            self.OutputLogger("Pending state returned for block_round {}".format(pending_state))
    ...
    def _validate_signatures(self, qc, last_tc):
        #TODO
        #self.OutputLogger("Safety validation" + str(type(qc)))
