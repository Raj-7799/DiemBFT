...
class Safety:
    ...
    def safe_to_extend(self, block_round, qc_round, tc):
        return self.consecutive(block_round, tc.round) and qc_round >= max(tc.tmo_high_qc)
    ...
    def make_vote(self, b, last_tc):
        ...
        if self.valid_signatures(b, last_tc) and self.safe_to_vote(b.round, qc_round, last_tc):
            ...
            exec_state_id = self.ledger.pending_state(b.id)

            ledger_commit_info = LedgerCommitInfo()
            ledger_commit_info.commit_state_id = self.commit_state_id_candidate(b.round, b.qc)
            ledger_commit_info.vote_info_hash = self.hash(b.round, b.qc)

            return VoteMsg(vote_info, ledger_commit_info, self.BlockTree.high_commit_qc)
