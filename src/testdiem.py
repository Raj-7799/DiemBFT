...
import da
_config_object = {}
from crypto.GenerateKey import GenerateKey
from crypto.Keys import Keys
from client.client import Client
from diem_core import diem_replica
from diem_core.replica_info import ReplicaInfo
...
class RunDiemBFT(da.DistProcess):
    ...
    def setup(self, config, **rest_452):
        super().setup(config=config, **rest_452)
    ...
    def run(self):
        ...
        for (i, client) in enumerate(self._state.clients):
            self._setup(self._state.clients, (i, self._state.requests, self._state.clientTimeout, replicaInfos, KeySets[(- 1)].private_key))
...
class Node_(da.NodeProcess):
    ...
    def run(self):
        configs = [{'faultyReplicas': 1, 'timeoutDelta': 500, 'clients': 1, 'requests': 4, 'clientTimeout': 5000, 'networkByzantine': False, 'networkByzantineSeed': 10, 'processByzantine': False, 'processByzantineSeed': 20}]
