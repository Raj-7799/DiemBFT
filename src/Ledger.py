...
class Ledger():
    ...
    def speculate(self,prev_block_id, block_id, txns):
        self.diem_logger.info("[Ledger][replicaID {}] START speculate ".format(self.replicaID))
        block_id=bytes(str(block_id),'utf-8')
        value = bytes(str([prev_block_id,txns]),'utf-8')        
        self._db_speculate.put(block_id,value)
        self.diem_logger.info("[Ledger][replicaID {}] END speculate ".format(self.replicaID))



    #find the pending state for the given block id or ⊥ if not present
    def pending_state(self,block_id):
        self.diem_logger.info("[Ledger][replicaID {}] START pending_state ".format(self.replicaID)) 
        block_id = bytes(str(block_id),'utf-8')     
        entry = self._db_speculate.get(block_id)
        if entry is not None:
            self.diem_logger.info("[Ledger][replicaID {}] END pending_state ".format(self.replicaID)) 
            return block_id
        self.diem_logger.info("[Ledger][replicaID {}] END pending_state ".format(self.replicaID)) 
        return None                
    ...
    def commit(self,block_id):
        self.diem_logger.info("[Ledger][replicaID {}] START commit ".format(self.replicaID)) 
        block_id = bytes(str(block_id),'utf-8')
        entry = self._db_speculate.get(block_id)        
        if  entry is not None:
            self._db.put(block_id,entry)
            self._db_speculate.delete(block_id)      
        self.diem_logger.info("[Ledger][replicaID {}] END commit ".format(self.replicaID)) 
             


    #returns a committed block given its id
    def committed_block(self,block_id):
        self.diem_logger.info("[Ledger][replicaID {}] START committed_block ".format(self.replicaID)) 
        block_id=bytes(str(block_id),'utf-8')
        self.diem_logger.info("[Ledger][replicaID {}] END committed_block ".format(self.replicaID)) 
